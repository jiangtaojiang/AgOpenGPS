...
namespace AgOpenGPS
{
    ...
    public class ShapeFile
    {
        ...
        {
            FileStream MainStream = File.Open(FilePath, FileMode.Open, FileAccess.Read, FileShare.Read);
                ...
                if (FileCode == 9994 && FileVersion == 1000 && MainStream.Length == FileLength)
                {
                    ...
                    while (FileLength > Index + ShapeLength)
                    {
                        ...
                        if (ShapeType == 5 && ShapeLength >= 52)
                        {
                            ...
                            if (aa.BndBoxLeft - 0.5 <= mf.Longitude && aa.BndBoxRight + 0.5 >= mf.Longitude)
                            {
                                if (aa.BndBoxTop - 0.5 <= mf.Latitude && aa.BndBoxBottom + 0.5 >= mf.Latitude)
                                {
